/* global React */
// Timeline component — 40-year scrubber with event markers
...
const EVENT_ROWS = (() => {
  ...
  const rows = new Map();
  let lastRowTime = [-1, -1]; // last t on row 0, row 1
  sorted.forEach(e => {
    // pick row that was last used furthest in the past
...
function Timeline({ t, setT, playing }) {
    ...
    <div className="gw-timeline">
      ...
      <div className="gw-tl-track-wrap">
        <div className="gw-tl-track" ref={trackRef} onClick={onTrack}
          ...
          {years.map(y => {
            const p = (y-1985)/40;
            return <div key={y} className="gw-tl-tick" style={{left: `${p*100}%`}}>
              <span>{y}</span>
            </div>;
          })}
          {EVENTS.map(e => {
            ...
            return (
              <button key={e.label+e.year} className={`gw-tl-ev gw-tl-ev-r${row}`} style={{left: `${e.t*100}%`}}
